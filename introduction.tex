...
\begin{itemize}
  ...
  \item WTF is a junction
  \item superconducting qubits, differences and capabilities
  \item phase architecture
  \item Defects, glasses, corrundum etc
  \item TLSs, differences btwn 1/f and strongly coupled
  \item Qubits as Probes of TLSs
\end{itemize}
...
\caption[Bloch Sphere]{\label{fig:bloch}Bloch Sphere where $\ket{\psi} = \cos\left(\frac{\theta}{2}\right) \ket{0} \, + \, e^{i \phi}  \sin\left(\frac{\theta}{2}\right) \ket{1} = \cos\left(\frac{\theta}{2}\right) \ket{0} \, +  \, ( \cos \phi + i \sin \phi) \, \sin\left(\frac{\theta}{2}\right) \ket{1}$ with $0 \leq \theta \leq \pi$ and $0 \leq \phi < 2 \pi$.}
\end{figure}
