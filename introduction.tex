...
On the other hand, this design is very useful for studying these decoherence sources (see \cref{sec:probes} below) and therefore the most relevant architecture to study throughout this thesis. 
...
This symbol represents the Josephson junction: the only non-dissipative, strongly non-linear circuit element available at low temperature.

A normal LC circuit is a harmonic oscillator as a result of the parabolic potential, meaning transitions between neighboring excited states are degenerate. 
If we wish to use the circuit as a qubit, the transition between state $\ket{0}$ and $\ket{1}$ must be sufficiently different to the higher-lying eigenstates of the system.

TODO: Josephson Effect etc
