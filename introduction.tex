...
In certain limits, the noise profiles described by Block-Redfield can be mapped to a Linbladian form []\xxx{Cite Jan}, which indicates that the relationship between these two theories is tightly bound.

Extensive experimentation on superconducting qubits has identified that at low frequencies, the spectral density of the noise exhibited be the environment varies approximately as $1/f\,$ over a sampled frequency range $f\,$~\cite{Dutta1981}, which in terms of decoherence times adds a contribution to the dephasing mechanism $\gamma$ in $T_2$ \cref{eq:t1t2}.



Essentially a $1/f$ \cite{Dutta1981,Shnirman2005} discussion, look at \cite{Enss2005} probably..
...
\footnote{It is interesting to note that historically, the tunneling of atoms was considered as a phenomena before electron tunneling or $\alpha$ particle emission was ever discussed.}
