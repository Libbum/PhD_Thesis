...
* Energy relaxation or state mixing occurs when ``the tip of the Bloch vector diffuses in the latitude direction, i.e. along the are joining the two poles of the sphere to or away from the North pole.'' \xxx{Unsure that this is correct}

\subsection{Decoherence and Dephasing}
\emph{Section can come from \cite{Makhlin2001, Paladino2014} perhaps.
Need to introduce decoherence as a major noise source.}
\xxx{Maybe ask Jan for a good primer, or scan his thesis.}

Quantum computation requires qubits to be strongly inter-coupled by control gates, but at the same time must be completely isolated (decoupled) from the surrounding environment.
Except of course; when control and I/O is required, and information must flow in and out of the computer.
...
The nitrogen vacancy (NV$^-$) center~\cite{Gruber1997} is arguably the most studied crystalline defect in existence.
...
Exhibiting atom-like properties: long-lived spin quantum states and well-defined optical transitions whilst encased in a robust solid-state device.
...
Even early schemes hinted at the possibility of coupling more than 100 qubits using a single cavity mode and laser fields to mediate coherent interactions between distant quantum dot spins~\cite{Imamoglu1999}.

\subsection{Superconducting Circuits}
    Charge qubit~\cite{Makhlin2001,Vion2002} (comes from the cooper pair box)~\cite{Nakamura1999,Bouchiat1998}
    Transmon\cite{Wallraff2004, Schreier2008} (essentially an improved charge qubit)
    Flux qubit~\cite{Mooij1999,VanderWal2000,Chiorescu2003}%~\cite{Plourde2005, Deppe2007}
    (comes from RF-SQUID)~\cite{Barone1982}
    Phase qubit (current-biased junction)

``On the other hand, the technology needs to be scalable. Solid-state systems have a clear advantage in this regard, offering the opportunity to adapt nanofabrication techniques developed in the semiconductor industry to build integrated quantum devices.''

* Superconducting circuits allow electrons to flow with almost no resistance at very low temperatures.

* Different from the previous architectures which rely on naturally isolated microscopic degrees of freedom, and instead use a collective electrodynamic mode of macroscopic electrical elements.
Gating is no longer an issue, as simple electrical elements (capacitors, inductors etc) can be coupled to the qubits directly.
Downside is the system is no longer easily isolated.

* Basic features: Ultra-low dissipation (from superconductivity); ultra-low noise (from low temperature); non-linear, non-dissipative elements (from the JJ);
...
* ``Degrees of freedom must be cooled to temperatures where the typical energy $kT$ of thermal fluctuations is much less that the energy quantum $\hbar \omega_{01}$ associated with the transition between the $\ket{0}$ and $\ket{1}$ states.
$\ldots$ this frequency is in the $5-20$ GHz range, thus the operating temp $T$ must be around $20$ mK (recall that $1$ K corresponds to about $20$ GHz).''

$kT \ll \hbar \omega_{01}, \quad \hbar \omega_{01} \ll \Delta$, where $\Delta$ is the energy gap of the superconducting material. \xxx{later we say $\Delta$ is the `tunneling energy'. Check this is equivalent.}
...
* the only non-dissipative, strongly non-linear circuit element available at low temperature.
...
\subsection{Qubits as Probes of TLSs}


\section{Historical Defect Research}


Condensed matter defects and thier properties have been an active area of research for many decades.
...
The two-level system (TLS) description in the STM requires two parameters: the tunneling energy $\Delta$ and asymmetry energy $\epsilon$.\xxx{this may need to go higher and be reworked here}
