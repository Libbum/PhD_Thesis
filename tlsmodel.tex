...
%
%Sampling $f(x)$ at $N$ absciss\ae (where $N$ is odd) equidistant from $x^*$ such that
%
%\begin{equation}
...
\section[Understanding Central Differences]{Understanding the Central Difference Method\footnote{While most of this and the following section is standard cannon information for finite differences; it follows \citeauthor{Mathur2012}, which will be formally introduced in \cref{sec:hopt}.}}
...
\begin{equation}
 C_n(x) = \frac{\mathcal{F}_n^{\,(d)}(x,h_2) - \mathcal{F}_n^{\,(d)}(x,h_1)}{h_1^n - h_2^n}.\label{eq:cdtrcoeff}
\end{equation}
...
\begin{equation}
\mathcal{TE}_n(x,h_1) = C_n(x)h_1^n = \frac{\mathcal{F}_n^{\,(d)}(x,h_2) - \mathcal{F}_n^{\,(d)}(x,h_1)}{h_1^n - h_2^n}h_1^n.\label{eq:cdte}
\end{equation}
