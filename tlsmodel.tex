...
\end{equation}
using the mass of an oxygen atom, which can now also be used to solve \cref{eq:Mlimit} for $M = 1.847\ten{98}$.
...
On various \texttt{Linux} distributions and \texttt{Windows 7}, \texttt{Matlab 2014b} provides two values for $\epsilon$: single precision $\epsilon_s = 1.192\ten{-7}$ and double $\epsilon_d = 2.220\ten{-16}$, for which double is the standard when memory and computational limitations are not a concern (an embedded processor may use single precision for instance).

All the components needed to find the optimal step size are now available. 
Recall \cref{eq:stepgen}
\begin{equation}
h = \left(\frac{30464\cdot 2.220\ten{-16}}{27\cdot 1.847\ten{98}}\right)^{1/8} = 1.385\ten{-14} \text{m} \approx 1\ten{-4} \text{Å}.
\end{equation}

Unfortunately, the solution is not as simple as that. 
The error calculated using \cref{eq:cdifferrtot} represents the error in $\cramped{\frac{\partial^2 \Psi}{\partial x^2}}$ from \cref{eq:hamti}, not the total error, nor the error in the energy $E$.

\xxx{Everything below here is sketchy as fuck}
\begin{equation}
\mathcal{E}_{tot} = \abs{\frac{\hbar^2}{2m}\mathcal{E}_T}+\abs{\mathcal{E}_V}
\end{equation}

is a closer representation of the complete error in \cref{eq:hamti}, where $\mathcal{E}_T$ and $\mathcal{E}_V$ are the errors associated with the kinetic and potential terms of the hamiltonian. 
The potential only suffers from roundoff errors, hence $\mathcal{E}_V = \mathcal{O}(\epsilon)$.
Considering \cref{eq:cdifferrtot} takes round off into account, $\mathcal{E}_{tot} \approx \mathcal{E}_T$ (with the kinetic scaling factor).
\Cref{fig:errvh} shows how $\mathcal{E}$ behaves over a range of $h$.
Truncation error dominates at large values, whereas roundoff error takes hold at small values. 
Note that while the double precision optimal value is a smaller (thus more computationally intensive) value, the error in the result is much smaller than the single precision result. 

\begin{figure}[htp]
\centering
\hspace{-1cm}\resizebox{0.7\columnwidth}{!}{\includestandalone{figures/Errvh}}
\caption[Step size optimisation for single and double precision floats]{\label{fig:errvh}Step size optimisation for single and double precision floats against the kinetic energy error $\mathcal{E}_T$.}
\end{figure}
