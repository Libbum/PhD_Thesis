...
yielding a value for $k = 526.321$.
Using the relationship in \cref{eq:vharm} for $\omega$ (where  $\cramped{m=m_{oxy}=2.657\ten{-26}}$ kg), $\omega=1.408\ten{14}$ and a complete equivalency has been found.
\Cref{fig:harmapprox} shows the approximation for this case.

\begin{figure}[htp]
\centering
\hspace{-1cm}\resizebox{0.7\columnwidth}{!}{\includestandalone{figures/harmapprox}}
\caption[Harmonic Approximation to Strietz Mintmire]{\label{fig:harmapprox}SM vs Harmonic}
\end{figure}

As discussed in \xxx{section discussing $\Lambda$ and GR}, an appropriate grid range which captures all of the important physics of the investigated clusters is $\pm 4.5$ Å around a central origin.

\begin{equation}
M = \mathrm{max}_{[-4.5 \text{Å},4.5 \text{Å}]}\left(\abs{\Psi_0^{(8)}(x)}\right) = 5.411\ten{17}
\end{equation}
