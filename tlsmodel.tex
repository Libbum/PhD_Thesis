...
\begin{equation}
h_{opt} = \left[\frac{d}{n}\frac{1}{\abs{C_n}}\left(\epsilon\abs{\mathcal{F}_\epsilon}+
\delta\abs{\mathcal{F}_\delta}\right)\right]^{1/(n+d)}.\label{eq:cdhopt}
\end{equation}

Note that obtaining correct values of $\mathcal{F}_\epsilon$ and $\mathcal{F}_\delta$ require the optimal step size $h_{opt}$, thus an iterative method is required.
...
\caption[3 point Error]{\label{fig:hopt3pt}Three point truncation error. Red $h_{opt}^U$, Green $h_{opt}$. Light small blue is base 10 data for comparison, stars are invalid roundoff (=0).}
\end{figure}
...
\caption[7 point Error]{\label{fig:hopt7pt}Seven point truncation error. Red $h_{opt}^U$, Green $h_{opt}$. Light small blue is base 10 data for comparison, stars are invalid roundoff (=0).}
\end{figure}
