...
can be connected to the Streitz Mintmire potential through the relationship $V(x_{max})=V_{SM}(x_{max})$, where at a maximum distance $x_{max}$, the harmonic potential is equivalent to Streitz Mintmire. As discussed in \xxx{Section on $\Lambda$}, $V_{SM}$ is measured in [μeV] with $x$ in [Å], whereas $V$ is measured in [J] against [m]. Unit transformations will therefore need to be carried through this derivation.
