...
The value of this corrected step is $h_{opt,true} \simeq 3.717\ten{-5}$ Å, which does not have an accurate power of two representation.
...
\begin{equation}
h_{opt} = 3.717\ten{-5} \simeq 2^{-15} \;\text{Å}.
\end{equation}
...
\caption[Step size optimisation of $f_2^{\;(6)}(x)$]{\label{fig:hopt7pt}Step size optimisation of $f_2^{\;(6)}(x)$ for step sizes $2^{-29}\!\leq\! h\! \leq\! 2^7$ \plotmarker{0.4}{circle,fill=Set1-5-2}. Smaller markers \plotmarker{0.25}{circle,fill=Set1-5-2!50} represent values of $\cramped{h \neq \left\{2^\mathds{N}\right\}}$. Two optimal step sizes are identified: $h_{opt,\mathcal{TE}}$ \plotmarker{0.5}{circle,fill=Set1-5-1}, found using the optimal step algorithm~\cite{Mathur2012}, and $h_{opt,true}$ \plotmarker{0.5}{circle,fill=Set1-5-3}, corrected by \cref{eq:hoptt}.}
\end{figure}
