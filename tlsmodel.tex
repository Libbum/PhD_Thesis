...
\section[Understanding Central Difference]{Understanding the Central Difference Method}
...
\begin{equation}
\mathcal{F}_n^{\,(d)}(x,h) = \frac{\Delta f_n^{\;(d)}(x,h)}{h^d} + \mathcal{O}(h^n).
\end{equation}
...
\section{Second Derivative of Order 2}
...
The simplest arrangement truncates the Taylor series at $\mathcal{O}(h^2)$ and takes the form

\begin{align}
f^{\;\prime\prime}(x) &= \frac{f(x-h)-2f(x)+f(x+h)}{h^2} - \frac{f^{\;(4)}(\xi)}{12}h^2, \quad \xi \in [x-h,x+h] \\
\mathcal{F}_2^{\,(2)}(x,h) &= \frac{f(x-h)-2f(x)+f(x+h)}{h^2} + \mathcal{O}(h^2).
\end{align}
