...
As expected, steps with high round off error contributions cause fluctuations in the calculated value of $E$, and invalid truncation error also generate large deviations from the acceptable value at $h_{opt} = 2^{-9}$.
\begin{figure}[htp]
\centering
\resizebox{0.9\columnwidth}{!}{\includestandalone{figures/Econverge}}
\caption[Energy Convergence]{\label{fig:econv}Energy Convergence for step sizes $10^{-7}\!\leq\! h\! \leq\! 10^0$ \plotmarker{0.4}{circle,fill=Set1-5-2}. The valid truncation range from \cref{fig:hopt7pt} is visible in the sense that large step sizes $h\gtrsim2\ten{-1}$ contribute sizable error. The inset shows round off error to causing an unwieldily response at small step sizes. }
\end{figure}
...
Thus $h_{max} = 2^{-3} \simeq 0.125$ Å for the sixth order second derivative from \cref{fig:hopt7pt} so long as $f^{\;(8)}(\xi)$ is constant over $[2^{-9},2^{-3}]$ which is indeed the case. 
\Cref{fig:econv} also validates this argument as the error contribution between $[h_{opt},h_{max}]$ to $E$ is minimal.
%TODO: Update fig1.5 with calculated data from overnight run
% Calculate a2 and a3. Are they 3?
