...
\section{The Josephson Junction Formation Process}
...
\begin{figure}[tbh]
\centering
%\includegraphics[width=0.85\textwidth]{AlO125_075.pdf}
...
\begin{figure}[tbh]
\centering
%\includegraphics[width=0.7\textwidth]{groptis}
\caption{\label{fig:groptis}Evolution of the oxygen projected radial distribution function $G(r)$. Crystalline corundum (thin, gray); Metal---metal--oxide interface reconstruction before (dash dotted, green) and after (dashed, blue); final optimised geometry (thick, red).  Inset: Oxygen projected $G(r)$ computed using \textit{ab initio}
 (\sw{VASP}) and empirical (\sw{GULP}) methods, showing no statistically significant differences.}%
\end{figure}
...
The inset of \cref{fig:groptis} compares the optimal $G(r)$ results for both the \sw{VASP} and \sw{GULP} simulations.
Whilst these results are very similar, the \sw{GULP} simulation actually produces a drastically different final structure.
...
In an attempt to normalise this response, we present a total energy per atom value in \Cref{fig:energy}, scaling against stoichiometry (left) and density (right).

\begin{figure}[tbh]
\centering
%\includegraphics[width=\textwidth]{energyperatom}
...
\begin{figure}[tbh]
\centering
%\includegraphics[width=\textwidth]{coord_hists.pdf}
