...
This system exhibits TLS-like behaviour, with $E_{01} = 40.63$ MHz, and a dipole strength in $y$ of $0.26 \; e$\AA: perpendicular to the confining $x$ axis as expected\footnote{$E_{01} \ll E_{12}$ is a TLS in the model sense, although experimentally $E_2$ of this system should be observable. If this system had a degenerate $E_{23}$ pair, it may hint to another reason why quad degenerate systems are not observed: resolution of the spectroscopy may not be precise enough and is actually measuring an anti-crossing coupled to $E_{12}$.}.
...
On way to check this assumption (and consequently further validate the TLS model) is to calculate the potential landscape seen by a single oxygen atom from bulk corundum ($\alpha$--\ce{Al_2O_3}).
...
\xxx{Jared suggests strained polytopes, wavefunctions and eigenspectrom in the figure}
